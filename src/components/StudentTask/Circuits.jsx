...
const Circuit = () => {
	...
	useEffect(() => {
		const circuitIdFromQuery = query?.get("circuit");
		if (circuitIdFromQuery && circuitsList) {
			const indexToExpand = circuitsList?.findIndex(
				(f) => f?.id === circuitIdFromQuery,
			);

			if (indexToExpand >= 0) {
				setExpanded(indexToExpand);
				getSingleCircuitDetails(circuitIdFromQuery);
			}
		}
	}, [circuitsList]);
	...
	const getCircuitsList = async (visibility = ["public"]) => {
		try {
			setListLoading(true);

			const response = await GET_CIRCUITS_LIST({
				visibility,
				is_visible: [true],
			});

			setCircuitsList(response?.data?.circuits);
		} catch (e) {
			console.error("Error fetching circuits list:", e);
		} finally {
	...
	const handleChange = (panel, id) => async (_event, isExpanded) => {
		...
		if (isExpanded && selectedCircuit?.id !== id) {
			getSingleCircuitDetails(id);
		}
	};

	const getSingleCircuitDetails = async (id) => {
		setLoading(true);
		try {
			const response = await GET_CIRCUIT_BY_ID(`/${id}`);
			setSelectedCircuit(response);
		} catch (error) {
			console.error("Error fetching selectedCircuit:", error);
		} finally {
			setLoading(false);
	...
	const handleProceed = () => {
		navigate(
			`/case/${selectedCase?.caseData?.id}?circuit=${selectedCircuit?.id}&station=${selectedCase?.stationId}`,
		);
	};
	...
	return listLoading ? (
		...
		<div className="d-flex flex-column gap-3">
			{circuitsList?.map((item, idx) => (
				<Accordion
					key={`circuits-circuit-list-${item?.id}-${idx}`} // Must be unique as we are using index and circuit id
				...
				>
					...
					<AccordionDetails className="m-0 p-0 px-3 pb-3">
						{loading && selectedCircuit?.id !== item?.id ? (
							...
							<>
								...
								<strong className="py-2">Stations:</strong>
								<div className="d-flex flex-column gap-3">
									{isEmpty(selectedCircuit?.stations) ? (
										<div className="text-center py-5">No Data Found</div>
									) : (
										selectedCircuit?.stations?.map((data, idx) => {
											const item = data;
											return (
												<div
													key={`circuit-selected-circuit-station-${idx + 1}-${item?.id}`} // Must be unique as we are using index and station id
												...
												>
													<div className="p-2 border-bottom d-flex gap-5 align-items-center justify-content-between">
														<div>{item?.name}</div>
														<div>Progress: {getProgress(item?.cases)}%</div>
													</div>
													<div className="p-2">
														<strong>Description: </strong>
														{item?.description}
													...
													<div className="p-2">
														...
														<div className="d-flex flex-wrap gap-3 pt-2">
															{isEmpty(item?.cases)
																? "no case available"
																: item.cases.map((caseData, idx) => (
																		<SingleCaseWithDialog
																			key={`circuit-single-case-with-dialog-${idx + 1}`} // Must be unique as we are using index
																			caseData={caseData}
																			handleSelectCase={() => {
																				handleSelectCase(caseData, item?.id);
																			}}
		...
		<>
			<div
				className={`border border-3 rounded rounded-pill 
					p-2 px-3 hover-scale d-flex gap-3 align-items-center 
					${
